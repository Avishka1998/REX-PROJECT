...
?>
<?php 
                    if(isset($_GET['cus_c_id']) && isset($_GET['cus_studio_id']) && isset($_GET['cus_complaint_id'])){
                        $c_id=$_GET['cus_c_id'];
                        $studio_id=$_GET['cus_studio_id'];
                        $complaint_id=$_GET['cus_complaint_id'];
                        $query="UPDATE customer_complaint SET flag='1' WHERE c_id=$c_id AND studio_id=$studio_id AND complaint_id=$complaint_id";
                        $result_set=mysqli_query($connection,$query);
                        //isset($_GET['studio_id'])
                    }
                    if(isset($_GET['stud_c_id']) && isset($_GET['stud_studio_id']) && isset($_GET['stud_complaint_id'])){
                        $c_id=$_GET['stud_c_id'];
                        $studio_id=$_GET['stud_studio_id'];
                        $complaint_id=$_GET['stud_complaint_id'];
                        $query="UPDATE studio_complaint SET flag='1' WHERE studio_id=$studio_id AND c_id=$c_id AND complaint_id=$complaint_id";
                        $result_set=mysqli_query($connection,$query);
