<?php 
require_once('../../inc/connection.php');
session_start();
?>
<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="utf-8">
	<title>Inbox</title>
	<link rel="stylesheet" type="text/css" href="../../css/customer/cust_inbox.css">
</head>

<body>
	<div class="nav body" style="padding-left: 0;">
		<?php require_once('../../inc/cust_dash_navbar.php');?>
	</div>
	<div class="body">
	<div class="chat-container">
		<div class="search-container">
			<input type="text" placeholder="Search"/>			
		</div>
		<div class="conver-list">
			<div class="conversastion active">
				<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="Sasindu Subodhaka" />
				<div class="title-text">
					Sasindu Subodhaka giyfi uiyhfu yhuyfhuiyhfuyhu
				</div>
				<div class="createrd-date">
					Apr 1
				</div>
				<div class="conversastion-message">
					Hi sasindu ury7wryb76b 77 47 67 yugfyg 
				</div>
			</div>

			<div class="conversastion">
				<img src="../../img/customer/blank-profile-picture-973460_1280.jpg" alt="Sasindu Subodhaka" />
				<div class="title-text">
					Sasindu Subodhaka giyfi uiyhfu yhuyfhuiyhfuyhu
				</div>
				<div class="createrd-date">
					Apr 1
				</div>
				<div class="conversastion-message">
					Hi sasindu ury7wryb76b 77 47 67 yugfyg 
				</div>
			</div>

			<div class="conversastion">
				<img src="../../img/customer/studio1.png" alt="Sasindu Subodhaka" />
				<div class="title-text">
					Sasindu Subodhaka giyfi uiyhfu yhuyfhuiyhfuyhu
				</div>
				<div class="createrd-date">
					Apr 1
				</div>
				<div class="conversastion-message">
					Hi sasindu ury7wryb76b 77 47 67 yugfyg 
				</div>
			</div>

			<div class="conversastion">
				<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="Sasindu Subodhaka" />
				<div class="title-text">
					Sasindu Subodhaka giyfi uiyhfu yhuyfhuiyhfuyhu
				</div>
				<div class="createrd-date">
					Apr 1
				</div>
				<div class="conversastion-message">
					Hi sasindu ury7wryb76b 77 47 67 yugfyg 
				</div>
			</div>

			<div class="conversastion">
				<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="Sasindu Subodhaka" />
				<div class="title-text">
					Sasindu Subodhaka giyfi uiyhfu yhuyfhuiyhfuyhu
				</div>
				<div class="createrd-date">
					Apr 1
				</div>
				<div class="conversastion-message">
					Hi sasindu ury7wryb76b 77 47 67 yugfyg 
				</div>
			</div>

			<div class="conversastion">
				<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="Sasindu Subodhaka" />
				<div class="title-text">
					Sasindu Subodhaka giyfi uiyhfu yhuyfhuiyhfuyhu
				</div>
				<div class="createrd-date">
					Apr 1
				</div>
				<div class="conversastion-message">
					Hi sasindu ury7wryb76b 77 47 67 yugfyg 
				</div>
			</div>

			<div class="conversastion">
				<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="Sasindu Subodhaka" />
				<div class="title-text">
					Sasindu Subodhaka giyfi uiyhfu yhuyfhuiyhfuyhu
				</div>
				<div class="createrd-date">
					Apr 1
				</div>
				<div class="conversastion-message">
					Hi sasindu ury7wryb76b 77 47 67 yugfyg 
				</div>
			</div>

			<div class="conversastion">
				<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="Sasindu Subodhaka" />
				<div class="title-text">
					Sasindu Subodhaka giyfi uiyhfu yhuyfhuiyhfuyhu
				</div>
				<div class="createrd-date">
					Apr 1
				</div>
				<div class="conversastion-message">
					Hi sasindu ury7wryb76b 77 47 67 yugfyg 
				</div>
			</div>

			<div class="conversastion">
				<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="Sasindu Subodhaka" />
				<div class="title-text">
					Sasindu Subodhaka giyfi uiyhfu yhuyfhuiyhfuyhu
				</div>
				<div class="createrd-date">
					Apr 1
				</div>
				<div class="conversastion-message">
					Hi sasindu ury7wryb76b 77 47 67 yugfyg 
				</div>
			</div>

			

			


		</div>
		<div class="new-message">
			<a href="#">+</a>
		</div>
		<div class="chat-title">
			<span>Sasindu Subodaka</span>
			<img src="../../img/customer/delete-128.png" alt="Delete Conversastion" height="30px" width="auto" />
		</div>
		<div class="chat-message-list">
			<div class="message-row you-message">
				<div class="message-content">
					<div class="message-text">
					Hi brooooooooo... hhtgj uhu gg gygy ygy yt gyy
					</div>
					<div class="message-time">Apr 15</div>
				</div>
				
			</div>
			<div class="message-row other-message">
				<div class="message-content">
					<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="attachement" height="40px" width="auto"/>
					<div class="message-text">
						Hi bro...
					</div>
					<div class="message-time">Apr 15</div>
					
				</div>
				
			</div>

			<div class="message-row you-message">
				<div class="message-content">
					<div class="message-text">
					Hi brooooooooo... hhtgj uhu gg gygy ygy yt gyy
					</div>
					<div class="message-time">Apr 15</div>
				</div>
				
			</div>
			<div class="message-row other-message">
				<div class="message-content">
					<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="attachement" height="40px" width="auto"/>
					<div class="message-text">
						Hi bro...
					</div>
					<div class="message-time">Apr 15</div>
					
				</div>
				
			</div>

			<div class="message-row you-message">
				<div class="message-content">
					<div class="message-text">
					Hi brooooooooo... hhtgj uhu gg gygy ygy yt gyy
					</div>
					<div class="message-time">Apr 15</div>
				</div>
				
			</div>
			<div class="message-row other-message">
				<div class="message-content">
					<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="attachement" height="40px" width="auto"/>
					<div class="message-text">
						Hi bro...
					</div>
					<div class="message-time">Apr 15</div>
					
				</div>
				
			</div>

			<div class="message-row you-message">
				<div class="message-content">
					<div class="message-text">
					Hi brooooooooo... hhtgj uhu gg gygy ygy yt gyy
					</div>
					<div class="message-time">Apr 15</div>
				</div>
				
			</div>
			<div class="message-row other-message">
				<div class="message-content">
					<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="attachement" height="40px" width="auto"/>
					<div class="message-text">
						Hi bro...
					</div>
					<div class="message-time">Apr 15</div>
					
				</div>
				
			</div>

			<div class="message-row you-message">
				<div class="message-content">
					<div class="message-text">
					Hi brooooooooo... hhtgj uhu gg gygy ygy yt gyy
					</div>
					<div class="message-time">Apr 15</div>
				</div>
				
			</div>
			<div class="message-row other-message">
				<div class="message-content">
					<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="attachement" height="40px" width="auto"/>
					<div class="message-text">
						Hi bro...
					</div>
					<div class="message-time">Apr 15</div>
					
				</div>
				
			</div>

			<div class="message-row you-message">
				<div class="message-content">
					<div class="message-text">
					Hi brooooooooo... hhtgj uhu gg gygy ygy yt gyy
					</div>
					<div class="message-time">Apr 15</div>
				</div>
				
			</div>
			<div class="message-row other-message">
				<div class="message-content">
					<img src="../../img/customer/109815470-man-avatar-profile-male-face-icon-vector-illustration-.jpg" alt="attachement" height="40px" width="auto"/>
					<div class="message-text">
						Hi bro...
					</div>
					<div class="message-time">Apr 15</div>
					
				</div>
				
			</div>

			
		</div>
		<div class="chat-form">
			<img src="../../img/customer/clip-512.png" alt="attachement" height="30px" width="auto"/>
			<input type="text" name="message" placeholder="type a message">
		</div>


	</div>
	</div>

</body>
</html>